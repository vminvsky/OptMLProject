...
from config import SENTIMENT_DATA_DIR
...
class SentimentDataset(DataClassWorkerVsGPT):
    ...
    def preprocess(self, model_name: str) -> None:
        ...
        self.data = self.data.map(
            lambda x: tokenizer(x[self.text_column], truncation=True, padding=True),
            batched=True,
        )
