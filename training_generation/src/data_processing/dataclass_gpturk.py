...
class GPTurkDataset(DataClassWorkerVsGPT):
    def __init__(
            self,
    ...
    def preprocess(self, model_name: str) -> None:
        ...
        #     lambda x: {"labels": mapping[x["labels_og"]]},
        #     remove_columns=["labels_og", "HITId"]
        # )
        # print(self.data["train"][0])

        self.data = self.data.map(
            lambda x: {"labels": self._label_preprocessing(x["labels_og"])},
            remove_columns=["HITId"]
        )
        ...
        self.data = self.data.map(
            lambda x: tokenizer(x[self.text_column], truncation=True, max_length=64, padding="max_length"),
            batched=True,
        )
