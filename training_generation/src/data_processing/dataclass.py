...
class DataClassWorkerVsGPT(Dataset):
    ...
    def __init__(self, path: Union[Path, None], is_augmented: bool = False, replace: bool = False) -> None:
        super().__init__()      
        if replace:
            temp = pd.read_json(path, orient="records")
            temp = temp.sample(frac=1, replace=True)
            self.data: datasets.DatasetDict = Dataset.from_pandas(temp)        
        else:
