...
def main(cfg: TrainerConfig) -> None:
    ...
    if SAM_ACTIVE:
        ...
        optimizer = SAM(model.parameters(), base_optimizer, lr=lr, rho=cfg.sam_rho)
        scheduler = StepLR(optimizer, lr, 2)

    else:
        optimizer = optimizer = torch.optim.AdamW(model.parameters(), lr=cfg.lr)
    ...
    # Training loop
    model.train()
    for epoch in range(cfg.num_epochs): # 3 epochs
